...
(define-mir-structs
  ...
  (const (slot (v :type :general) (type-name :type :general)))
...
;; 	(%current-module-table #f *inherit*))
  ...
  (make-imm (slot (type-name :type :general)
		  v))
...
(define (%make-const v . opt-type)
  (define (f v t) (values v (symbol-append 'scm: t)))
  (receive (v t)
      (case v
	((#t true)  (f #t 'bool))
	((#f false) (f #f 'bool))
	((() nil null) (f '() 'nil))
	((eof undef unbound) (f v v))
	(else
	 (case/pred
	  v
	  (integer? (f v 'integer))
	  (else (values v #f)))))
    (make-const :v v :type-name (or t (get-optional opt-type #f)))))

(define (make-asm-int x) (%make-const x 'asm:integer))
(define (make-scm-int x) (%make-const x 'scm:integer))
